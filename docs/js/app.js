...
import { filter as ffilter, map as fmap, reduce as freduce, pipe, toLower, between } from './fp.js';
...
function filterAutos(items, { q, minPrice, maxPrice }) {
  const qn = toLower(q.trim());
  const min = Number(minPrice) || 0;
  const max = Number(maxPrice) || 0;
  const byText = (a) => qn === '' || toLower(`${a.marca} ${a.modelo}`).includes(qn);
  const inRange = (a) => between(min, max, a.precio);
  return pipe(
    ffilter(byText),
    ffilter(inRange),
  )(items);
